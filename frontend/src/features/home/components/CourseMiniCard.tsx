...
import { IconUsers, IconMessageCircle, IconCards, IconHelpCircle } from '@tabler/icons-react';
...
export function CourseMiniCard({ course }: { course: CourseListItem }) {
  ...
  return (
    <Paper shadow="sm" radius="md" style={{ cursor: 'pointer', overflow: 'hidden', minWidth: 220 }} onClick={() => navigate(`/courses/${course.courseId}`)}>
      <div style={{ height: 140, background: getCourseCoverBackground(course.coverImage) }} />
      <Stack gap={4} p="sm">
