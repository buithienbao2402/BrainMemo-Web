...
export interface MediaBlockDraft extends BaseBlockDraft {
    ...
    rawFile: File | null;
    /** URL tuyệt đối để preview — KHÔNG gửi lên server. */
    previewUrl: string | null;
    /**
     * objectKey server trả về sau khi upload (vd "/uploads/images/abc.png").
     * Đây là giá trị gửi lên BE qua field `mediaUrl` khi lưu chương.
     * Luồng Sửa: được hydrate từ media_url đang lưu trong DB.
     */
...
interface ChapterBuilderState extends ChapterDraftSnapshot {
    ...
    updateBlock: (
        ...
        blockTempId: string,
        patch:
            | Partial<Pick<TextBlockDraft, 'contentText'>>
            | Partial<Pick<MediaBlockDraft, 'rawFile' | 'previewUrl' | 'mediaKey' | 'blockType'>>
    ) => void;
...
function createEmptyBlock(blockType: BlockType): BlockDraft {
    ...
    return { blockTempId: crypto.randomUUID(), blockType, rawFile: null, previewUrl: null, mediaKey: null };
}
