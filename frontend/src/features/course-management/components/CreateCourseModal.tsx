...
export function CreateCourseModal({ opened, onClose, course, onDeleted }: CreateCourseModalProps) {
    ...
    // Khóa vùng ảnh bìa khi đang lưu hoặc đang upload
    const coverLocked = isSubmitting || isUploadingCover;
    ...
    useEffect(() => {
        ...
        if (course) {
            form.setValues({
                ...
                // GIỮ key hiện tại — nếu để '' thì khi lưu sẽ ghi đè làm mất ảnh bìa cũ
                coverImageObjectKey: course.coverImageUrl ?? '',
                tags: course.tags,
                accessType: course.accessType,
                passcode: '',
                status: course.status,
            });
            setCoverPreview(resolveMediaUrl(course.coverImageUrl));
        } else {
            form.reset();
    ...
    const applyCoverFile = async (file: File | null) => {
        ...
        if (!file.type.startsWith('image/')) {
            notifications.show({ title: 'Tệp không hợp lệ', message: 'Chỉ chấp nhận tệp ảnh.', color: 'red' });
            return;
        }

        const previousPreview = coverPreview;
        const localUrl = URL.createObjectURL(file);
        setCoverPreview(localUrl); // hiện ngay trong lúc upload
        setIsUploadingCover(true);
        try {
            const { objectKey, url } = await uploadMedia(file, 'IMAGE');
            form.setFieldValue('coverImageObjectKey', objectKey);
            setCoverPreview(url);
        } catch (err) {
            setCoverPreview(previousPreview); // rollback về ảnh trước đó
            notifications.show({ title: 'Tải ảnh bìa thất bại', message: getMediaErrorMessage(err), color: 'red' });
        } finally {
    ...
    const handleFileInputChange = (event: ChangeEvent<HTMLInputElement>) => {
        void applyCoverFile(event.target.files?.[0] ?? null);
    ...
    const handleDrop = (event: DragEvent<HTMLDivElement>) => {
        ...
        setIsDragOver(false);
        void applyCoverFile(event.dataTransfer.files?.[0] ?? null);
    };

    const handleClose = () => {
        if (isSubmitting || isDeleting || isUploadingCover) return;
        form.reset();
        clearCover();
        setDeleteConfirmOpen(false);
        onClose();
                ...
                <Text size="sm" fw = { 500} mb = { 2} >
                    Ảnh bìa khóa học
                        ...
                        < Text size = "xs" c = "dimmed" mb = { 8} >
                            Khuyến nghị 1280x720, JPG / PNG / WebP & lt; 5MB
                                </Text>
    ...
    onChange = { handleFileInputChange }
        ...
        <Box
              onClick={ coverLocked ? undefined : openFilePicker }
    onDragOver = {(event) => {
        event.preventDefault();
        if (!coverLocked) setIsDragOver(true);
    }
}
onDragLeave = {() => setIsDragOver(false)}
onDrop = { coverLocked? undefined: handleDrop }
style = {{
    border: `1px dashed ${isDragOver ? 'var(--mantine-color-orange-6)' : 'var(--mantine-color-default-border)'
        ...
        borderRadius: 'var(--mantine-radius-md)',
            backgroundColor: isDragOver ? 'var(--mantine-color-orange-light)' : 'var(--mantine-color-body)',
                cursor: coverLocked ? 'default' : 'pointer',
                    overflow: 'hidden',
...
color = "red"
onClick = { clearCover }
disabled = {!coverPreview || coverLocked}
              >
    Xóa ảnh
        </Button>
        </Group>
...
<Group>
    <Button
                variant="default"
onClick = { handleClose }
disabled = { isSubmitting || isDeleting || isUploadingCover}
              >
    Hủy
    </Button>
    < Button
...
disabled = { isDeleting || isUploadingCover}
              >
